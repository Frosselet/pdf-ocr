...
- **`pdf_extractor.py`** — PDF table extraction facade. Thin wrapper around `compress.py` aligning the API with other extractors: `extract_tables_from_pdf()` delegates to `compress_spatial_text_structured()`, `pdf_to_markdown()` renders as pipe-table markdown. Per-module docs in `PDF_EXTRACTOR.md`.
